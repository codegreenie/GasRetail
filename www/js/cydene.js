...
var exitMyApp, getImg4rmGallery, fireUpPayments, fireUpPayments2, shareCydene, showStatusBar;
...
shareCydene = function(){

// this is the complete list of currently supported params you can pass to the plugin (all optional)
var options = {

  message: 'Order gas to be delivered to your home with Cydene Express. Download here:', // not supported on some apps (Facebook, Instagram)
  subject: 'Cydene Express', // fi. for email
  files: ['', ''], // an array of filenames either locally or remotely
  url: 'https://bit.ly/2gXZLEM',
...
StatusBar.backgroundColorByHexString("#0d6e9e");
		...
		function uploadImage(theImage){
			var thePhone = window.localStorage.getItem("_cydene_user_phone_no");
			    ...
			    var win = function (r) {
				    ...
				    $$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/finish_seller_logo_upload.php",
						{
				...
				ft.upload(theImage, encodeURI("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/upload_seller_logo.php"), win, fail, options);
...
myApp.onPageInit('theswipe', function(page){
	...
	//window.setTimeout(function(){showStatusBar();}, 1000);
		...
		$$('#user_tel').on('keyup', function(e){
			...
			$$("#verify_reg_arrow").on('click', function(e){
				...
				showNylon();
						...
						$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/verify_user.php",
						{
...
/**********************Apply OTP existing User Page *****************/
	...
	myApp.onPageInit('addotp', function(page){
			...
			$$("#verify_existing_reg_arrow").on('click', function(e){
						...
						$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/verify_otp.php",
						{
			...
			$$("#resend_otp").on("click", function(){
				...
				$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/resend_otp.php",
						{
	...
	function fbLogin() {
		...
		facebookConnectPlugin.api("/me?fields=email,name,picture", ["public_profile","email"], function(userData){
			...
			showNylon();
					...
					$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/buyer_registration.php",{
...
$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_wallet_balance.php",
	{
...
$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_homepage_order_history.php",
	{
	 ...
	 function(data){
	 	...
	 	if(the_status == "record found"){
		 	...
		 	for(p = 1; p < data.length; p++){
		 		...
		 		$$("#" + data[p].myDiv + "-junior").html("<img src=https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/imgs/cylinder_yellow.png style='max-width: 20px;'><br>" + cylinder_size + "Gas<br><br>Quantity: " + cylinder_qty);
		 		$$("#" + data[p].myDiv).show();
		...
		$$("#fivekggas").on("click", function(){
			...
			showNylon();
			toast.show("Please enable location services. GPS");
			storeBuyDetails("5kg");
		...
		if(!window.localStorage.getItem("version_control") || window.localStorage.getItem("version_control") != "1.0.6"){


			window.localStorage.setItem("version_control", "1.0.6");
		}
		...
		$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/version_control.php", function(datax){
					...
					buttons : [
						{
							...
							onClick : function(){
								...
								toast.show("Please update Cydene Express");
							}
						},
						{
							...
							onClick : function(){
								...
								window.open("https://bit.ly/2gXZLEM");
							}	
...
/**********************Sellers*****************/
var buyFromThisSeller, getBuyQty, pushMyAddrToBox;
	myApp.onPageInit('sellers', function(page){



		$$("#manual-address-film").hide();
	...
	$$("#switch-2-manual-address").on("click", function(){

		myApp.closeModal('.picker-info');
		$$("#manual-address-film").show();
		runAddressSuggestions();
	
	});


	$$("#manual-delivery-address").on("keydown, focus", function(){

		initAutoComplete();
		$$("#my-addresses-container").html("&nbsp;");
		
	});


	function runAddressSuggestions(){

		$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/my_address_suggestions.php",
		{"account_owner" : window.localStorage.getItem("_cydene_user_phone_no")},
		function(data){

			$$("#my-addresses-container").html(data);

			var mySwiper2 = myApp.swiper('.address-suggest-swipe', {
			   spaceBetween: 10,
			  slidesPerView: 1.5
			});
			...
			toast.show("Error getting address suggestions");
		});
	...
	var input = document.getElementById('manual-delivery-address');
	var options = {
		  types: ['address'],
...
var finalAddressInUse = {
	"the_address_string" : "Ikeja, Lagos",
	"the_lat" : 6.345,
	"the_lng" : 3.533
};
...
pushMyAddrToBox = function(fullAddr){

	$$("#manual-delivery-address").val(fullAddr);
}
								...
								function getMapLocation() {
								    ...
								    (onMapSuccess, onMapError, { enableHighAccuracy: true, timeout : 30000, maximumAge : 3000 });
								}
								...
								var onMapSuccess = function (position) {
								    ...
								    watchMapPosition();
										...
										var receivedLatLng = {lat : Latitude, lng : Longitude};
										var geocoder = new google.maps.Geocoder();
											    geocoder.geocode( { 'location': receivedLatLng}, function(results, status) {
											      if (status == 'OK') {
											        
											        	if (results[0]) {

													       var stringAddress = results[0].formatted_address;
													       finalAddressInUse.the_address_string = stringAddress;
													       finalAddressInUse.the_lat = Latitude;
													       finalAddressInUse.the_lng = Longitude;


													       console.log(JSON.stringify(finalAddressInUse));

													     } else {
													        toast('Geocoding your current location failed!');
													      }
											    	...
											    	hideNylon();
													toast.show("Google could not geocode address. Try again later");
						    	...
						    	url : "https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/imgs/location_arrow.png",
					...
					function pushMap(){
							...
							var locations = JSON.parse(window.localStorage.getItem("returnJson"));
						      ...
						      var image = {
						    	url : "https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/imgs/cylinder_yellow.png",
						    ...
						    for (i = 0; i < themArray.length; i++){  
								...
								finalAddressInUse.the_lat = myPosition[0];
								finalAddressInUse.the_lng = myPosition[1];
								window.setTimeout(function(){findMySupplier();}, 5000);
						...
						function pushMap2(uLat, uLng){
						    ...
						    for (i = 0; i < themArray.length; i++){  
								   ...
								   var sellersAddress = new google.maps.LatLng(themArray[i][1], themArray[i][2]);
								   var myAddress = new google.maps.LatLng(uLat, uLng);
								...
								finalAddressInUse.the_lat = uLat;
								finalAddressInUse.the_lng = uLng;
								finalAddressInUse.the_address_string = $$("#manual-delivery-address").val();
						...
						function setMyAddressPosition(uLat, uLng){

							var newLatLng = new google.maps.LatLng(uLat, uLng);
							marker.setPosition(newLatLng);
							map.setCenter(marker.getPosition());
						...
						findMySupplier = function(){
								var sortDistanceList = distanceLists.sort(function(a, b){return a-b});
						...
						window.setTimeout(function(){getMapLocation();}, 1000);
	...
	function getAllSellers(){
		...
		$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/find_sellers.php", 
			{
...
$$("#find-this-address").on("click", function(){

	showNylon();
	$$("#my-addresses-container").html("&nbsp;");

	var geocoder = new google.maps.Geocoder();
		    geocoder.geocode( { 'address': $$("#manual-delivery-address").val()}, function(results, status) {
		      if (status == 'OK') {
		        hideNylon();
		       theAddressLatLng = results[0].geometry.location;
		        ...
		        setMyAddressPosition(theAddressLat, theAddressLng);
				    	...
				    	val = parseInt(val);
				    		...
				    		if(Number.isInteger(val) == false){
				    			...
				    			var merry = {
				    				...
				    				"gasQty" : val,
				    				"delivery_address" : finalAddressInUse.the_address_string
				    			}
		...
		buyFromThisSeller = function(theSellerID, theCylinderSize){
			...
			$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_seller_details.php", {"seller_id" : theSellerID, "cylinder_size" : theCylinderSize}, function(data){
...
var couponDetails = JSON.parse(window.localStorage.getItem("coupon_details"));
		...
		$$(".populate-sellers-logo").attr("src", "https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/imgs/" + splitSellerDetails.seller_details_logo);
		...
		$$("#apply-coupon").click(function(){
			couponSN = 25;
					...
					myApp.prompt('Enter Coupon Code', 'Cydene Coupons!', function (value) {
       					 ...
       					 showNylon();
       					 $$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/coupon_code_validator.php",
								 {
		...
		$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/relay_date.php", 
			function(data){
		...
		if(couponDetails){
			...
			dynamicCoupon = 0;
				...
				if(paymentMethod == "COD"){
					...
					$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/transaction_recorder.php", 

					{
						...
						"tnx_delivery_date" : $$("#modify-delivery-date").val()
					}, 
...
myApp.onPageInit('pinexec', function(page){
		...
		$$("#exec-arrow").click(function(){
			...
			showNylon();
			$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/validate_exec_pin.php", 
				{
...
myApp.onPageInit('mapexp', function(page){
	...
	$$("#save-address-btn").on("click", function(){
		...
		$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/save_delivery_address.php",
						{
...
myApp.onPageInit('orderhistory', function(page){
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_order_history.php", 
	{
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_scheduled_order_history.php", 
	{
		"the_buyer" : window.localStorage.getItem("_cydene_user_phone_no")
	},
	 function(data){
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_active_orders.php", 
	{
		"the_buyer" : window.localStorage.getItem("_cydene_user_phone_no")
	},
	 function(data){
	...
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/transaction_complete.php", 
	{
		"tranx_id" : tranxID
	},
	 function(data){
	...
	showNylon();	
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/cancel_scheduled_order.php", 
	{
		"order_sn" : tranxSN
	},
	 function(data){
...
else if(tranxPaymentType == "Card")
{
	...
	function cancelMyOrder(tranxSN, paymentMethod){
			...
			showNylon();
			$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/cancel_order.php", 
			{
				"order_sn" : tranxSN,
...
myApp.onPageInit('settings', function(page){
	...
	$$("#share-cydene").on("click", function(){
...
myApp.onPageInit('addresseslist', function(page){ // Address List
	...
	var users_phone = window.localStorage.getItem("_cydene_user_phone_no");
		...
		$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/list_addresses.php", 
		{
	...
	showNylon();	
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/delete_address.php", 
	{
		"address_sn" : addrSN
	},
	 function(data){
		      ...
		      if (status == 'OK') {
		       ...
		       theAddressLatLng = results[0].geometry.location;
		        ...
		        var theAddressLng = theAddressLatLng.lng();
				 ...
				 $$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/update_delivery_address.php",
				{
...
myApp.onPageInit('wallet', function(page){ // Wallet page
	...
	$$("#add-money-2-wallet").on("click", function(){
		...
		$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/add_money_2_wallet_transaction.php",
		{
		...
		var returnedTnx = splitData[0];
			...
			if(splitData[1] == "Successful"){
				...
				$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/paystack_init.php",
					{
...
myApp.onPageInit('sellerwallet', function(page){ // Wallet page
	...
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_seller_wallet_balance.php",
	{
		"user_phone_data" : user_phone_data
	},
	 function(data){
	...
	$$("#request-payout-btn").on("click", function(){
		var thePayoutAmount = $$("#payout-amount").val();
			...
			$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/request_seller_payout.php",
					{
						"user_phone_data" : user_phone_data,
...
myApp.onPageInit('editprofile', function(page){ // Edit Profile page
	...
	$$("#profile-edit-btn").on("click", function(x){
			...
			x.preventDefault();
			showNylon();
			$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/user_profile_update.php", 
			{
...
myApp.onPageInit('offers', function(page){ //Offers page
	...
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_offers.php",
	 function(data){
...
myApp.onPageInit('walletstatement', function(page){ //Offers page
	...
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_wallet_statement.php",
	{
...
myApp.onPageInit('sellerdashboard', function(page){ //Offers page
	...
	$$("#seller-logo-space").attr("src", "https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/imgs/" + sellerLogo);
	...
	var mycurrentVersion = window.localStorage.getItem("version_control");
		...
		$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_get_verification_status.php", 
		{
		...
		//Pull seller ratings
		$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_seller_ratings.php", 
		{
...
myApp.onPageInit('sellerbookings', function(page){ //Offers page
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_pull_pending_orders.php", 
	{
		"the_seller" : window.localStorage.getItem("_cydene_user_phone_no")
	},
	 function(data){
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_pull_active_orders.php", 
	{
		"the_seller" : window.localStorage.getItem("_cydene_user_phone_no")
	},
	 function(data){
	...
	showNylon();	
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_accept_order.php", 
	{
		"order_sn" : tranxSN
	},
	 function(data){
	...
	var declineSheet = [
		...
		myApp.actions(declineSheet);
			...
			function letsDeclinOrder(theTranxSN, theDeclineReason){
				...
				showNylon();	
				$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_decline_order.php", 
				{
					"order_sn" : tranxSN,
	...
	var declineSheet = [
		...
		myApp.actions(declineSheet);
			...
			function letsDeclinSchOrder(theTranxSN, theDeclineReason){
				...
				showNylon();	
				$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_decline_schedule_order.php", 
				{
					"order_sn" : tranxSN,
					"decline_reason" : theDeclineReason
				},
	...
	pushSaleDetails = function(theBuyer, tranxID, tranxSN){
		...
		$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_sale_details.php", 
		{
			"the_buyer" :  theBuyer,
			"tranx_ID" :  tranxID,
			"tranx_SN" : tranxSN
	...
	pushScheduleSaleDetails = function(theBuyer, tranxSN){
		...
		$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_schedule_sale_details.php", 
		{
			"the_buyer" :  theBuyer,
			"tranx_SN" : tranxSN
	...
	showNylon();	
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_accept_schedule_order.php", 
	{
		"order_sn" : tranxSN
	},
	 function(data){
...
schOrderComplete = (tranxID) => {
	...
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/sellers_schedule_transaction_complete.php", 
	{
		"tranx_id" : tranxID
	},
	 (data) => {
...
myApp.onPageInit('sellerorderhistory', function(page){ //Sellers Order History
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_sellers_order_history.php", 
	{
		"the_seller" : window.localStorage.getItem("_cydene_user_phone_no")
	},
	 function(data){
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/pull_sellers_scheduled_order_history.php", 
	{
		"the_seller" : window.localStorage.getItem("_cydene_user_phone_no")
	},
	 function(data){
...
myApp.onPageInit('sellersettings', function(page){ //Sellers Settings
	...
	$$("#the-seller-logo-space").attr("src", "https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/imgs/" + theSellerLogo);




	var users_phone_data = window.localStorage.getItem("_cydene_user_phone_no");
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_sellers_wallet_balance.php",
	{
	...
	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_sellers_availability.php",
	{
		"user_phone_data" : users_phone_data
	},
	 function(data){
		...
		var sheet = [
...

			{
				...
				onClick : function(){
					...
					$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/sellers_set_availability.php",
						{
							"user_phone_data" : users_phone_data,
		...
		var geocoder = new google.maps.Geocoder();
		    geocoder.geocode({ 'address': theDeliveryAddress}, function(results, status) {
		      if (status == 'OK') {
		       ...
		       $$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_registration.php",
				{
...
myApp.onPageInit("updatepricelist", function(page){
	...
	$$.getJSON("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_sellers_current_pricelist.php",
		{
...
myApp.onPageInit("selleraddbankaccount", function(page){
   ...
   var bankRoad = ['Citi Bank','Diamond Bank', 'Zenith Bank', 'United Bank Of Afica', 'Guarantee Trust Bank', 'Access Bank Plc', 'First Bank Nigeria', 'Ecobank', 'Fidelity Bank', 'First City Monument Bank', 'Heritage Bank', 'ASO Savings and Loans', 'Coronation Merchant Bank', 'FBN Mortgages Limited', 'Fortis Microfinance Bank', 'FSDH Merchant Bank', 'Imperial Homes Mortgage Bank', 'Jaiz Bank', 'Jubilee Life Mortgage Bank', 'Keystone Bank', 'New Prudential Bank', 'Nigeria International Bank(CITIGROUP)', 'NPF Microfinance Bank', 'Omoluabi Savings and Loans Plc', 'Page MFBank', 'Parallex MFB', 'Safetrust Mortgage Bank', 'Skye Bank Plc', 'Stanbic IBTC Bank', 'Standard Chattered Bank', 'SunTrust Bank', 'Trustbond Mortgage Bank', 'Union Bank of Nigeria', 'Unity Bank Plc', 'VFD Microfinance Bank Plc', 'Wema Bank Plc', 'Sterling Bank'];
        ...
        $$("#submit-add-bank-account").on("click", function(){
        	...
        	$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/seller_add_bank_account.php",
				{
...
myApp.onPageInit('paymentmethods', function(page){ //Payment Methods Page
	...
	$$.get("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/fetch_buyer_cards.php",
	{
		"user_phone_data" : users_phone_data
	},
	 function(data){
	...
	deleteCardForever = function(cardSN){
		showNylon();
		$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/delete_card.php",
			{
				"user_phone_data" : users_phone_data,
				"card_serial" : cardSN
			},
	...
	if(window.localStorage.getItem("discounted_price")){
		...
		$$("#continue-2-paystack-btn").on("click", function(){
			...
			showNylon();
					 		...
					 		$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/card_transaction_recorder.php", 
...
$$("#the-rate").val("1");
	...
	$$("#send-rating").click(() => {
		...
		$$.post("https://ourdailymanna.org/Mobile_app_repo/php_hub/_GasRetail/rate_seller.php",
				{
